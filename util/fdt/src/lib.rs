...
impl FdtNode {
        ...
        while match structs.next() {
            Some((i, 1)) => {
                ...
                while match name.next() {
                    ...
                    None if 0 == unsafe { *sptr.offset(j) } || b'@' == unsafe{ *sptr.offset(j) } => {
    ...
    pub fn prop<T>(&self, name: &str) -> &'static [T] {
        let mut structs = self.structs.iter().map(|x| u32::from_be(*x)).enumerate();
        while match structs.next() {
            Some((_, 1)) => {
                while match structs.next() {
                    ...
                    None if bytes[j] == 0 => {
                        return unsafe {
                            core::slice::from_raw_parts(
                                self.structs[(i + 3)..].as_ptr() as *const T,
                                (len as usize) / core::mem::size_of::<T>(),
                            )
                        };
                    }
